...
import { IBreakpoint } from '../../services/DebugSession.js';

// Ensure DebugSessionManager is available globally
declare global {
...
export class StartDebugSessionTool extends BaseTool {
  ...
  async execute(params: unknown, context: IToolContext): Promise<IToolResponse> {
    ...
    try {
      ...
      // Prepare breakpoints
      const breakpoints: IBreakpoint[] = input.breakpoints?.map((bp, index) => ({
      ...
      if (!input.executionId && input.executionData) {
        const execution = await context.apiClient.triggerWorkflow(
          input.workflowId,
          input.executionData
        );
