...
export class TimelineDebugTool extends BaseTool {
  ...
  async execute(params: unknown, context: IToolContext): Promise<IToolResponse> {
    ...
    try {
      const timeline = input.timeRange
        ? session.getTimelineRange(
            input.timeRange.start ? new Date(input.timeRange.start) : undefined,
            input.timeRange.end ? new Date(input.timeRange.end) : undefined
          )
        : session.getTimeline();
  ...
  private generateTextTimeline(timeline: any[], includeData: boolean): string {
    const lines: string[] = [];
  ...
  private generateMermaidTimeline(timeline: any[], workflowId: string): string {
    const lines: string[] = [];
  ...
  private calculateStatistics(timeline: any[]): {
    totalNodes: number;
    uniqueNodes: number;
    successful: number;
    failed: number;
