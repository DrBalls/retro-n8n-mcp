...
export class CompareVersionsTool extends BaseTool {
  ...
  private formatDiffAsMarkdown(diff: any, fromVersion: any, toVersion: any, options: any): string {
    const lines = [
    ...
    if (diff.operations.length === 0) {
      ...
      diff.operations.forEach((op: any, index: number) => {
  ...
  private formatDiffAsText(diff: any, fromVersion: any, toVersion: any, options: any): string {
    const lines = [
      `Workflow Version Comparison`,
      `==========================`,
      '',
  ...
  private operationToReadableText(operation: any): string {
  ...
  private pathToReadable(path: string): string {
    const parts = path.split('/').filter(p => p);
    
    if (parts.length === 0) return 'root';
    if (parts[0] === 'nodes') return parts.length > 1 ? `node ${parts[1]}` : 'nodes';
    if (parts[0] === 'connections') return 'connections';
    if (parts[0] === 'name') return 'workflow name';
    if (parts[0] === 'active') return 'activation status';
