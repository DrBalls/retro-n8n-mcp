...
export class WorkflowMapTool extends BaseTool {
  ...
  private generateJSONMap(
    ...
    execution: z.infer<typeof ExecutionSchema> | null,
...
<body>
  ...
  <defs>
    ...
    jsonMap.connections.forEach((conn: any) => {
      const sourceNode = jsonMap.nodes.find((n: any) => n.id === conn.source);
      const targetNode = jsonMap.nodes.find((n: any) => n.id === conn.target);
    ...
    // Draw nodes
    svg += '  <g id="nodes">\n';
    jsonMap.nodes.forEach((node: any) => {
  ...
  private countConnections(connections: any): number {
    let count = 0;
    Object.values(connections).forEach((targets: any) => {
      Object.values(targets).forEach((outputs: any) => {
  ...
  private calculateBounds(nodes: any[]): any {
  ...
  private calculateDensity(nodes: any[], connections: any[]): number {
    if (nodes.length <= 1) return 0;
    const maxConnections = nodes.length * (nodes.length - 1);
    return connections.length / maxConnections;
  }

  private getDataTransferInfo(sourceId: string, targetId: string, executionData: any): any {
  ...
  private identifyCriticalPath(nodes: any[], connections: any[]): string[] {
  ...
  private getNodeColor(node: any): string {
    if (node.execution?.status === 'error') return '#FFEBEE';
    if (node.execution?.status === 'success') return '#E8F5E9';
    if (node.category === 'trigger') return '#FFF3E0';
    if (node.category === 'webhook') return '#E3F2FD';
    return '#FFFFFF';
  }
