...
export class GetCredentialTool extends BaseTool {
  ...
  async execute(params: unknown, context: IToolContext): Promise<IToolResponse> {
    ...
    try {
      ...
      const usage = workflows.data.reduce((acc: any[], workflow: any) => {
        const nodes = workflow.nodes || [];
        const usingNodes = nodes.filter((node: any) => 
          node.credentials && 
      ...
      const result = {
        ...
        usage: {
          ...
          activeWorkflows: usage.filter((u: any) => u.active).length,
          workflows: usage
        },
