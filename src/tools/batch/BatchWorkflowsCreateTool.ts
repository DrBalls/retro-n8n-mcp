...
import { IBatchWorkflowCreate, IBatchOperationOptions } from '../../types/batch.types.js';
...
export class BatchWorkflowsCreateTool extends BaseTool {
  ...
  async execute(params: unknown, context: IToolContext): Promise<IToolResponse> {
    ...
    const workflowsToCreate: IBatchWorkflowCreate[] = input.workflows.map(workflow => ({
      name: workflow.name,
    ...
    if (input.options?.validateWorkflows) {
      batchOptions.validateItem = async (item: IBatchWorkflowCreate) => {
        // Basic validation
    ...
    try {
      ...
      if (batchOptions.atomic && finalStatus.status === 'rolled_back') {
        response.rollback = {
          ...
          rolledBackCount: response.results.created.filter((w: any) => 
            finalStatus.items.find((i: any) => i.result?.id === w.workflowId && i.status === 'rolled_back')
          ).length,
        };
      }
