...
export class CacheMonitoringService extends EventEmitter {
  private readonly logger = new Logger('CacheMonitoringService');
  ...
  private invalidationHistory: CacheInvalidation[] = [];
  ...
  constructor(cacheManager: MultiTierCacheManager) {
    ...
    this.cacheManager.on('cache:event', (event: CacheEvent) => {
      this.recordEvent(event);
    });
    
    this.cacheManager.on('cache:invalidation', (invalidation: CacheInvalidation) => {
   ...
   */
  async getPerformanceSummary(timeframeMs: number = 3600000): Promise<{
  ...
  private evaluateAlertCondition(rule: AlertRule, metrics: CacheMetrics): boolean {
    try {
  ...
  private createAlert(rule: AlertRule, metrics: CacheMetrics): void {
    const alert: Alert = {
  ...
  private generateAlertMessage(rule: AlertRule, metrics: CacheMetrics): string {
    switch (rule.name) {
      case 'low_hit_rate':
  ...
  private recordInvalidation(invalidation: CacheInvalidation): void {
