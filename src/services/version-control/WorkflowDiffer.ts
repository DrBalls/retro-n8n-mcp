import { VersionDiff, DiffOperation } from '../../types/version-control.types.js';
import { Workflow } from '../../types/n8n.types.js';
import { generateId } from '../../utils/generateId.js';

/**
 * JSON Patch Operation for internal use
 */
...
export class WorkflowDiffer {
  ...
  getModifiedNodes(fromWorkflow: Workflow, toWorkflow: Workflow): Array<{
    ...
    const modified: Array<{ nodeId: string; changes: string[] }> = [];
  ...
  private generateJsonPatch(from: any, to: any, path = ''): JsonPatchOperation[] {
  ...
  private generateArrayPatch(from: any[], to: any[], path: string): JsonPatchOperation[] {
    const operations: JsonPatchOperation[] = [];
  ...
  private generateObjectPatch(from: any, to: any, path: string): JsonPatchOperation[] {
    const operations: JsonPatchOperation[] = [];
  ...
  private convertToVersionDiffOperation(op: JsonPatchOperation): DiffOperation {
    // Filter out 'test' operations which are not part of our DiffOperation type
    if (op.op === 'test') {
      return {
        operation: 'add' as const, // Convert test to add for compatibility
        path: op.path,
        value: op.value,
        oldValue: undefined,
        from: undefined,
    ...
    return {
      operation: op.op as DiffOperation['operation'],
      path: op.path,
      value: op.value,
  ...
  private getNodeChanges(fromNode: any, toNode: any): string[] {
