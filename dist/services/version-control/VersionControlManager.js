...
class InMemoryVersionStorage {
    ...
    async getVersions(workflowId, branch) {
...
export class VersionControlManager {
    logger = new Logger('VersionControlManager');
    storage;
    differ;
    constructor(storage) {
     ...
     */
    async initializeWorkflow(workflowId, workflow, settings) {
    ...
    async createVersion(workflowId, workflow, options = {}) {
        const { branch = 'main', message = 'Update workflow', author = 'user', versionType = 'auto', tags = [], isSnapshot = false, } = options;
        ...
        if (currentVersion) {
            ...
            newVersion = SemanticVersioning.getNextVersion(currentVersion.version, versionType, changeAnalysis);
        }
        else {
            // First version
    ...
    async createBranch(workflowId, branchName, options = {}) {
        const { fromBranch = 'main', fromVersionId, author = 'user', description, } = options;
     ...
     */
    async mergeBranch(workflowId, sourceBranch, targetBranch, options = {}) {
        const { message = `Merge ${sourceBranch} into ${targetBranch}`, author = 'user', strategy = 'auto', autoResolve = true, } = options;
        ...
        const mergeResult = await this.performThreeWayMerge(baseVersion?.workflow, sourceVersion.workflow, targetVersion.workflow, strategy, autoResolve);
        const result = {
            id: generateId(),
            sourceBranchId: source.id,
            targetBranchId: target.id,
     ...
     */
    async rollback(workflowId, targetVersionId, options = {}) {
        const { branch = 'main', message = `Rollback to version ${targetVersionId}`, author = 'user', createBackup = true, } = options;
    ...
    async getVersionHistory(workflowId, options = {}) {
        const { branch, limit = 50, offset = 0, author, since, until, } = options;
     ...
     */
    async generateVersionDiff(fromVersionId, toVersionId) {
    ...
    async updateBranchHead(workflowId, branchName, versionId) {
        const branches = await this.storage.getBranches(workflowId);
    ...
    async performThreeWayMerge(baseWorkflow, sourceWorkflow, targetWorkflow, strategy, autoResolve) {
...
//# sourceMappingURL=VersionControlManager.js.map