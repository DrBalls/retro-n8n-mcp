...
export class HealthCheck {
    ...
    timeout;
    checkFn;
    constructor(name, description, timeout, checkFn) {
        this.name = name;
...
export class HealthCheckManager {
    ...
    async runAllChecks() {
        const results = await Promise.all(Array.from(this.checks.values()).map(check => check.check().catch(error => ({
            name: check.name,
            status: HealthStatus.UNHEALTHY,
    ...
    static createStandardChecks(dependencies) {
        ...
        if (dependencies.apiClient) {
            checks.push(new HealthCheck('n8n_api', 'Check n8n API connectivity', 3000, async () => {
                try {
        ...
        if (dependencies.database) {
            checks.push(new HealthCheck('database', 'Check database connectivity', 2000, async () => {
                try {
        ...
        if (dependencies.redis) {
            checks.push(new HealthCheck('redis', 'Check Redis connectivity', 2000, async () => {
                try {
        ...
        // Memory usage check
        checks.push(new HealthCheck('memory', 'Check memory usage', 1000, async () => {
        ...
        checks.push(new HealthCheck('disk', 'Check disk space', 2000, async () => {
