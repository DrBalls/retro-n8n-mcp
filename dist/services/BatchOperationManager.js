...
export class BatchOperationManager extends EventEmitter {
    ...
    apiClient;
    logger = new Logger('BatchOperationManager');
     ...
     */
    async createBatchOperation(type, items, options = {}) {
     ...
     */
    async processBatchOperation(operationId, options) {
        const operation = this.operations.get(operationId);
        if (!operation) {
        ...
        const { stopOnError = false, atomic = false, concurrency = 5, maxRetries = 3, retryDelay = 1000, onProgress, validateItem, } = options;
        let transaction;
        if (atomic) {
        ...
        try {
            ...
            const results = await this.processItemsWithConcurrency(operation, concurrency, maxRetries, retryDelay, stopOnError, validateItem, onProgress);
            // Update final status
     ...
     * Process items with concurrency control
     */
    async processItemsWithConcurrency(operation, concurrency, maxRetries, retryDelay, stopOnError, validateItem, onProgress) {
        ...
        while (queue.length > 0 || processing.size > 0) {
            ...
            while (processing.size < concurrency && queue.length > 0) {
                ...
                const processPromise = this.processItem(operation, item, maxRetries, retryDelay, validateItem).then(() => {
     ...
     */
    async processItem(operation, item, maxRetries, retryDelay, validateItem) {
     ...
     */
    async rollbackItem(operationType, item) {
     ...
     */
    updateProgress(operation, onProgress) {
        const progress = {
            operationId: operation.id,
            type: operation.type,
     ...
     */
    cleanupOldOperations(olderThanMs = 3600000) {
