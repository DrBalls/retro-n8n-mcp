...
export class CacheWarmingService {
    logger = new Logger('CacheWarmingService');
    cacheManager;
    apiClient;
    warmingConfig;
    prefetchConfig;
    warmingTimers = new Map();
    isRunning = false;
    // Statistics
    stats = {
        totalWarmed: 0,
    ...
    constructor(cacheManager, warmingConfig, prefetchConfig, apiClient) {
     ...
     */
    async warmStrategy(strategyName) {
     ...
     */
    async triggerPrefetch(conditionContext) {
    ...
    // Private methods
    async scheduleWarmingStrategy(strategy) {
    ...
    async warmFromApi(strategy, keyBuilder) {
        if (!this.apiClient) {
    ...
    async warmFromDatabase(strategy, keyBuilder) {
    ...
    async warmFromPrecomputed(strategy, keyBuilder) {
        let warmed = 0;
    ...
    async executePrefetch(prefetchKeys, context) {
        let prefetched = 0;
        const keyBuilder = this.cacheManager.getKeyBuilder();
    ...
    async prefetchByKeyType(key, keyBuilder) {
        if (!this.apiClient) {
            return false;
        }
    ...
    resolveKeyPattern(pattern, context) {
