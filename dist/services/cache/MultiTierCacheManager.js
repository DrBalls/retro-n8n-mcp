...
export class MultiTierCacheManager extends EventEmitter {
    logger = new Logger('MultiTierCacheManager');
    config;
    keyBuilder;
    memoryLayer;
    redisLayer;
    ...
    async set(key, value, options) {
        ...
        try {
            ...
            if (this.config.strategy?.writeThrough) {
                ...
                const promises = this.layers.map(layer => layer.set(key, value, setOptions).catch(error => {
    ...
    async delete(key) {
        const startTime = Date.now();
        try {
            const promises = this.layers.map(layer => layer.delete(key).catch(error => {
     ...
     */
    async clear() {
        const promises = this.layers.map(layer => layer.clear().catch(error => {
    ...
    async invalidateByPattern(pattern, reason) {
        try {
            const promises = this.layers.map(layer => layer.invalidateByPattern(pattern).catch(error => {
     ...
     */
    async invalidateByTag(tag, reason) {
        try {
            const promises = this.layers.map(layer => layer.invalidateByTag(tag).catch(error => {
     ...
     */
    async invalidateWorkflow(workflowId, reason) {
     ...
     */
    async invalidateExecution(executionId, workflowId, reason) {
     ...
     */
    async getStats() {
        const layerStats = await Promise.all(this.layers.map(async (layer) => {
            try {
     ...
     */
    async close() {
        const promises = this.layers.map(layer => layer.close().catch(error => {
    ...
    async promoteToHigherLayers(key, entry, sourceLayer) {
        ...
        const promises = higherLayers.map(layer => layer.set(key, entry.value, { ttl: entry.ttl, tags: entry.tags }).catch(error => {
            this.logger.debug(`Failed to promote to ${layer.name} layer`, { key, error });
        }));
        await Promise.all(promises);
    }
    writeBehindToOtherLayers(key, value, options, excludeLayer) {
        // Write to other layers asynchronously
        setImmediate(async () => {
            const otherLayers = this.layers.filter(layer => layer !== excludeLayer);
            const promises = otherLayers.map(layer => layer.set(key, value, options).catch(error => {
                this.logger.debug(`Write-behind failed for ${layer.name} layer`, { key, error });
            }));
            await Promise.all(promises);
        });
    }
    emitCacheEvent(type, layer, key, value, latencyMs, error) {
        const event = {
    ...
    emitInvalidationEvent(strategy, target, reason) {
        const invalidation = {
            strategy,
            target,
            cascade: true,
...
//# sourceMappingURL=MultiTierCacheManager.js.map