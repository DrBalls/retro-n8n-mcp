...
export class DatabaseCacheLayer {
    name = 'database';
    config;
    logger = new Logger('DatabaseCacheLayer');
    ...
    async keys(pattern) {
        ...
        try {
            ...
            return results.map((row) => row.cache_key);
        }
        catch (error) {
            this.stats.errors++;
    ...
    async updateAccessTracking(key, accessedAt, accessCount) {
        try {
            if (this.db.prepare) {
                // better-sqlite3 API
...
//# sourceMappingURL=DatabaseCacheLayer.js.map