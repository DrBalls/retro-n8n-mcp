...
export class BatchWorkflowsDeactivateTool extends BaseTool {
    ...
    async execute(params, context) {
        ...
        const workflowsToDeactivate = input.workflowIds.map(id => ({ id }));
        // Pre-check workflows
        ...
        const checkPromises = workflowsToDeactivate.map(async (workflow) => {
            try {
                ...
                if (!workflowData.active) {
                    ...
                    try {
                        const executions = await context.apiClient.request('GET', '/executions', {
                            params: {
                ...
                for (const workflow of preCheckResults.runningExecutions) {
                    try {
                        ...
                        for (const execution of executions.data) {
                            try {
                                await context.apiClient.request('POST', `/executions/${execution.id}/stop`);
                                stopResults.stopped++;
                            }
                            catch (error) {
                                stopResults.failed++;
        ...
        const workflowsToProcess = workflowsToDeactivate.filter(w => !preCheckResults.alreadyInactive.find((inactive) => inactive.id === w.id));
        ...
        try {
            ...
            const operationId = await batchManager.createBatchOperation('deactivate', workflowsToProcess, batchOptions);
            // Wait for completion
            ...
            for (const item of finalStatus.items) {
                ...
                if (item.status === 'success') {
                    const workflowInfo = preCheckResults.readyToDeactivate.find((w) => w.id === workflowId);
...
//# sourceMappingURL=BatchWorkflowsDeactivateTool.js.map