...
export class BatchWorkflowsUpdateTool extends BaseTool {
    ...
    async execute(params, context) {
        ...
        // Prepare update data
        const workflowUpdates = input.updates.map(update => ({
            id: update.id,
            name: update.name,
            nodes: update.nodes,
        ...
        try {
            ...
            const operationId = await batchManager.createBatchOperation('update', workflowUpdates, batchOptions);
            ...
            for (const item of finalStatus.items) {
                if (item.status === 'success' && item.result) {
                    ...
                    if (update.name !== undefined)
                        updateSummary.fieldsUpdated.push('name');
                    if (update.nodes !== undefined)
                        updateSummary.fieldsUpdated.push('nodes');
                    if (update.connections !== undefined)
                        updateSummary.fieldsUpdated.push('connections');
                    if (update.settings !== undefined)
                        updateSummary.fieldsUpdated.push('settings');
                    if (update.staticData !== undefined)
                        updateSummary.fieldsUpdated.push('staticData');
                    if (update.tags !== undefined)
                        updateSummary.fieldsUpdated.push('tags');
                    if (update.active !== undefined)
                        updateSummary.fieldsUpdated.push('active');
...
//# sourceMappingURL=BatchWorkflowsUpdateTool.js.map