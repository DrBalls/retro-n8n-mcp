...
export class BatchWorkflowsActivateTool extends BaseTool {
    ...
    async execute(params, context) {
            ...
            return {
                content: [{
                        ...
                        text: JSON.stringify({
                            ...
                            duplicates: input.workflowIds.filter((id, index) => input.workflowIds.indexOf(id) !== index),
                        }, null, 2),
        ...
        const workflowsToActivate = input.workflowIds.map(id => ({ id }));
        // Pre-check workflows
            ...
            const checkPromises = workflowsToActivate.map(async (workflow) => {
                try {
                    ...
                    if (workflowData.active) {
                        ...
                        const hasCredentialIssues = workflowData.nodes?.some((node) => {
        ...
        const workflowsToProcess = workflowsToActivate.filter(w => !preCheckResults.alreadyActive.find((active) => active.id === w.id));
        ...
        try {
            ...
            const operationId = await batchManager.createBatchOperation('activate', workflowsToProcess, batchOptions);
            // Wait for completion
            ...
            for (const item of finalStatus.items) {
                ...
                if (item.status === 'success') {
                    const workflowInfo = preCheckResults.readyToActivate.find((w) => w.id === workflowId);
            ...
            if (input.options?.testRun && response.results.activated.length > 0) {
                ...
                for (const activated of response.results.activated) {
                    try {
                        const execution = await context.apiClient.request('POST', `/workflows/${activated.workflowId}/execute`, { data: { mode: 'manual' } });
...
//# sourceMappingURL=BatchWorkflowsActivateTool.js.map