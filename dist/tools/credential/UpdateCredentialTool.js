...
export class UpdateCredentialTool extends BaseTool {
    ...
    async execute(params, context) {
        ...
        try {
            ...
            const currentCredential = await context.apiClient.request('GET', `/credentials/${input.id}`);
            // Prepare update data
            const updateData = {
            ...
            const response = await context.apiClient.request('PATCH', `/credentials/${input.id}`, {
                data: updateData
            });
            const result = {
                id: response.data.id,
...
//# sourceMappingURL=UpdateCredentialTool.js.map