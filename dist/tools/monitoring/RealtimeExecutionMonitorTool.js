...
export class RealtimeExecutionMonitorTool extends BaseTool {
    ...
    async execute(params, context) {
        ...
        try {
            ...
            let finalExecution;
            try {
                finalExecution = await context.apiClient.request('GET', `/executions/${input.executionId}`, { params: { includeData: input.includeProgress } });
            }
            catch (error) {
     ...
     */
    generateSummary(updates, finalExecution) {
    ...
    extractNodeProgress(execution) {
        ...
        for (const [nodeId, nodeExecutions] of Object.entries(runData)) {
            ...
            nodeProgress.push({
                nodeId,
                nodeName: execution.workflowData?.nodes?.find((n) => n.id === nodeId)?.name || nodeId,
...
//# sourceMappingURL=RealtimeExecutionMonitorTool.js.map