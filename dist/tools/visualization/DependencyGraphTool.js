...
export class DependencyGraphTool extends BaseTool {
    ...
    generateJSONGraph(graph, workflow, options) {
        const nodes = [];
        const edges = [];
    ...
    generateDotGraph(graph, workflow, options) {
        const lines = [];
    ...
    generateMermaidGraph(graph, workflow, options) {
        const lines = [];
        lines.push('graph LR');
        lines.push('');
        // Define nodes
    ...
    findLongestPathFrom(startId, graph) {
    ...
    findParallelizationOpportunities(graph) {
        const opportunities = [];
        const levels = new Map();
    ...
    calculateHierarchicalLayout(nodes, graph) {
