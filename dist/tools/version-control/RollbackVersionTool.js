...
export class RollbackVersionTool extends BaseTool {
    ...
    async execute(params, context) {
        ...
        try {
            ...
            const targetVersion = await this.versionControlService.getVersionHistory(input.workflowId, { limit: 1000 }).then(versions => versions.find(v => v.id === input.targetVersionId));
            if (!targetVersion) {
                throw new Error(`Target version ${input.targetVersionId} not found`);
            }
            // Get current version for comparison
            const currentVersions = await this.versionControlService.getVersionHistory(input.workflowId, { branchName: input.branchName, limit: 1 });
            ...
            const rolledBackVersion = await this.versionControlService.rollbackToVersion(input.workflowId, input.targetVersionId, {
                branchName: input.branchName,
...
//# sourceMappingURL=RollbackVersionTool.js.map