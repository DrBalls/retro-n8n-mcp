...
export class VersionDiffTool extends BaseTool {
    ...
    logger = new Logger('VersionDiffTool');
    ...
    async execute(params, context) {
        ...
        try {
            ...
            // Generate diff
            const diff = await this.versionManager.generateVersionDiff(input.fromVersionId, input.toVersionId);
    ...
    analyzeNodeChanges(fromWorkflow, toWorkflow) {
        ...
        return {
            ...
            modified: modifiedNodes.map(change => ({
                ...
                node: fromWorkflow.nodes?.find((n) => n.id === change.nodeId),
            })),
        };
    }
    analyzeConnectionChanges(fromWorkflow, toWorkflow) {
    ...
    analyzeImpact(diff, nodeChanges, connectionChanges) {
        const impact = {
            level: 'low',
            score: 0,
            factors: [],
    ...
    formatSummaryDiff(diff, nodeChanges, connectionChanges) {
        return {
    ...
    formatVisualDiff(diff, fromWorkflow, toWorkflow) {
    ...
    formatDetailedDiff(diff, nodeChanges, connectionChanges, input) {
        return {
            ...
            nodeChanges: {
                ...
                modified: nodeChanges.modified.map((mod) => ({
                    ...mod,
    ...
    generateContextualInformation(diff, fromVersion, toVersion) {
        return {
            versionProgression: {
    ...
    generateOperationDescription(operation) {
    ...
    extractMetadataChanges(operations) {
        return operations.filter(op => op.path.includes('/name') ||
            op.path.includes('/active') ||
            op.path.includes('/tags') ||
            op.path.includes('/settings'));
    }
    isVersionUpgrade(fromVersion, toVersion) {
        if (toVersion.major > fromVersion.major)
            return true;
        if (toVersion.major === fromVersion.major && toVersion.minor > fromVersion.minor)
            return true;
        if (toVersion.major === fromVersion.major && toVersion.minor === fromVersion.minor && toVersion.patch > fromVersion.patch)
            return true;
        return false;
    }
    calculateVersionDifference(fromVersion, toVersion) {
        return {
...
//# sourceMappingURL=VersionDiffTool.js.map