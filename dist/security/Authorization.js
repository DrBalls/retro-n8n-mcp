...
export class Authorization {
    ...
    roles = new Map();
    users = new Map();
    resourcePermissions = new Map(); // resourceId -> userId -> permissions
    constructor() {
    ...
    hasPermission(context, permission) {
        ...
        if (context.user) {
            ...
            if (context.resource) {
                const resourcePerms = this.getResourcePermissions(context.resource.id, context.user.id);
     ...
     */
    getResourcePermissions(resourceId, userId) {
        const resourcePerms = this.resourcePermissions.get(resourceId);
        if (!resourcePerms) {
            return [];
        }
     ...
     */
    checkPermissionList(permissions, permission) {
        // Check for superuser
